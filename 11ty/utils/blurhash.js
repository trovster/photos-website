...
const encodeAc = ([red, green, blue], maximumValue) => {
  const redValue = Math.floor(
    Math.max(0, Math.min(18, Math.floor(signPow(red / maximumValue, 0.5) * 9 + 9.5))),
  )
  const greenValue = Math.floor(
    Math.max(0, Math.min(18, Math.floor(signPow(green / maximumValue, 0.5) * 9 + 9.5))),
  )
  const blueValue = Math.floor(
    Math.max(0, Math.min(18, Math.floor(signPow(blue / maximumValue, 0.5) * 9 + 9.5))),
  )
...
export const encodeBlurhash = (
  ...
  const actualMaximumValue = Math.max(
    ...ac.map((factor) => Math.max(...factor.map((value) => Math.abs(value)))),
  )
